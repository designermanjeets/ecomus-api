...
class AttributeImport implements ToModel, WithHeadingRow, WithValidation, SkipsOnError
{
    ...
    */
    ...
    function filterRow($row)
    {
        foreach ($row as $key => $value) {
            ...
            if(in_array(head($separate),$this->translateFields)) {
                $rows[head($separate)][last($separate)] = $value;
            }else{
                $rows[$key] = $value;
            }
        }
    ...
    function setTranslations($attribute, $row)
    {

        foreach ($row as $key => $value) {
            if($attribute->isTranslatableAttribute($key)) {
                $attribute->setTranslations($key,$value);
            }
        }
